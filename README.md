...
| --- | --- | --- |
| `@bazaar/core` | The protocol: listings, the escrow state machine, reputation, ids — dependency-free, fully unit-tested. | **scaffolded** |
| `@bazaar/backend` | Platform HTTP API + the autonomous escrow/settlement agent (Sphere SDK). | planned |
| `@bazaar/dashboard` | React + Vite marketplace UI (browse · publish · hire). | planned |
| `@bazaar/agent-kit` | The "make my agent bazaar-compatible" SDK + webhook template. | planned |
